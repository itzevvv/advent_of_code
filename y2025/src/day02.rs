...
fn block_check(input : &str) -> bool {
    for pos in 1..input.len() {
        let section = &input[..pos];

        if input.as_bytes().chunks(pos).all(|chunk| chunk == section.as_bytes()) {
            return true
        }
    }
...
pub fn day_two() {
    ...
    let input = common::read_file_as_string("input/y2025/day02.txt");

    let res_part01 = part_one(&input);
    println!("{res_part01}");

    let res_part02 = part_two(&input);
    println!("{res_part02}");
}